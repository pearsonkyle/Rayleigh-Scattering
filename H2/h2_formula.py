...
interp1d = lambda x,y,xnew : interpolate.interp1d(x,y,kind='linear')(xnew)
...
if __name__ == "__main__":
    ...
    # H2 cross section from Ford & Brown 1973
    x,y = np.loadtxt('h2rayleigh.dat').T

    # compute cross section with refraction measurements per wavelength
    frac = np.loadtxt('h2refraction.dat')
    a = polarizability(101325/(kb*273), frac[:,1] )
    cross = ray(frac[:,0]*1e-6,a, 0.02)*1e4 # m2*1e4 -> cm2
    res = minimize(fn2min,x0=[1e-12,0,0],method='Nelder-Mead',args=(frac[:,0]*1e4,cross,errorp,sigma_ray) )
    ...
    inits = [[8.14e-13,1.28e-6,1.61]]*2 + [[9.0e-13]]*2
    ...
    f.suptitle("Rayleigh Scattering Cross Section for H2",fontsize=18)
