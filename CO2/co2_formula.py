...
if __name__ == "__main__":
    ...
    x,y = np.loadtxt('co2rayleigh.dat').T

    # compute cross section with refraction measurements per wavelength
    frac = np.loadtxt('co2refraction.txt')
    a = polarizability(101325/(kb*273), frac[:,1] )
    cross = ray(frac[:,0]*1e-6,a, 0.09)*1e4 # m2*1e4 -> cm2
    res = minimize(fn2min,x0=[1e-11,0,0],method='Nelder-Mead',args=(frac[:,0]*1e4,cross,errorp,sigma_ray) )
    ...
    inits = [[8.14e-13,1.28e-6,1.61]]*2 + [[2e-11]]*2
    ...
    f.suptitle("Rayleigh Scattering Cross Section for CO2",fontsize=18)
